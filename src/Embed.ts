...
import { hexToInt, intToHex } from "./utils";
...
export class Embed {
  ...
  static from(rawEmbed: RawEmbed) {
    const embed = new Embed({
      ...
      url: rawEmbed.url,
      // color: intToHex(rawEmbed.color),
      description: rawEmbed.description,
    });

    if (rawEmbed.color) embed.setColor(intToHex(rawEmbed.color));
    if (rawEmbed.timestamp) embed.setTimestamp(new Date(rawEmbed.timestamp).getMilliseconds());
    if (rawEmbed.fields) embed.addFields(...rawEmbed.fields);
    if (rawEmbed.footer) embed.setFooter(rawEmbed.footer.text, rawEmbed.footer.icon_url);
    ...
    if (rawEmbed.author)
      embed.setAuthor(rawEmbed.author.name, rawEmbed.author.url, rawEmbed.author.icon_url);
